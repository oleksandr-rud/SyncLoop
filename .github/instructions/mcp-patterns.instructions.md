---
name: "SyncLoop: MCP Patterns"
description: "MCP server bootstrap, resources, tools, prompts, lifecycle, and transport patterns"
applyTo: "{src,bin}/**/*.{js,ts}"
---
...
for (const [id, doc] of Object.entries(DOCS)) {
  server.registerResource(doc.name, `syncloop://docs/${id}`, async (uri) => {
...
server.registerTool(
  "my-tool",
  {
    ...
    // inputSchema: validated automatically, exposed in tools/list
...
```js
server.registerTool("fetch-data", { url: z.string() }, async ({ url }, ctx) => {
...
| Document | Purpose |
|----------|---------|
| [patterns.md](patterns.instructions.md) | Pattern registry — this spec is registered here |
| [code-patterns.md](code-patterns.instructions.md) | P1–P11 general code patterns |
| [validate-env.md](validate-env.instructions.md) | MCP smoke test is the primary gate for this codebase |
